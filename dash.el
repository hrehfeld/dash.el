...
(defun dash--member-fn ()
  ...
  (let ((cmp -compare-fn))
    (cond ((memq cmp '(nil equal)) #'member)
          ...
          ((eq cmp #'eql) #'memql)
          ((lambda (element list)
             (while (and list (not (funcall cmp element (car list))))
               (pop list))
             list)))))
...
of LIST, as returned by `-frequencies'."
...
`-compare-fn' if that is non-nil."
  (cond ((null list) (list ()))
        ;; A traversal of `list' is faster than the round trip via
        ;; `dash--uniq-perms' or `dash--multi-perms'.
        ((dash--numbers<= list)
